...
#'
#' @details The function takes a \code{positions} object, which can be a spatial object or a matrix of coordinates, and adds them to each of the elements of \code{r_frame_list} using \link[moveVis]{add_gg}. Optionally it also adds text at their respective positions using\link[moveVis]{add_text}- \code{ts_add_positions_to_frames} is intended to be an easy way to add multiple objects to the spatial frames at fixed positions. 
#' For adding individual positions or text, potentially at varying positions, call \link[moveVis]{add_gg} and \link[moveVis]{add_text} directly.
#' @seealso \link[moveVis]{add_text} \link[moveVis]{add_gg}
#' @importFrom moveVis add_text add_gg
#' @importFrom sp coordinates
#' @author Johannes Mast
#' @return list of ggplots with added positions.
#' @export
